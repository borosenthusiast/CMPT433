...
#define JSPUSH "/sys/class/gpio/gpio27/value"
#define GPIO_EXPORT "/sys/class/gpio/export"

int pollJoyGPIO() {
    long seconds = 0;
    long nanoseconds = 10000000; //poll every 10ms
    ...
    FILE *pFilePush = fopen(JSPUSH, "r");
    if (pFileUp == NULL || pFileRight == NULL || pFileDown == NULL || pFileLeft == NULL || pFilePush == NULL) {
    ...
    while(true) {
        ...
        fgets(buffPush, MAX_LENGTH, pFilePush);
        ...
        else if (strcmp(buffPush, "0") == 0 || atoi(buffPush) == 0) {
            fclose(pFileUp);
            fclose(pFileRight);
            fclose(pFileLeft);
            fclose(pFileDown);
...
void exportGPIO() { //export GPIO pins for joystick
    ...
    fclose(pFile);
    nanosleep(&reqDelay, (struct timespec *) NULL);
    printf("Finished exporting GPIO Pins.\n");
}