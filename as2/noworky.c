...
{
	int i; // bug was here, the unsigned intger underflowed on decrement of 0, before the condition check of the for loop to 4294967295, the uint max, and also satisfying i>=0, and therefore
					// overflowing array1 and array2.
	for (i = size-1; i >= 0; i--) {
